...
public class KiwiFruitTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
    private static final BlockState DEFAULT_TRUNK = Blocks.OAK_LOG.getDefaultState();
    private static final BlockState DEFAULT_LEAF = ModBlocks.KIWI_FRUIT_LEAVES.getDefaultState().with(HarvestableLeaves.GROWN, false).with(LeavesBlock.PERSISTENT, false).with(LeavesBlock.DISTANCE, 1);
    protected final int minTreeHeight;
    private final boolean vinesGrow;
    private final BlockState trunk;
    private final BlockState leaf;

    public KiwiFruitTreeFeature(Function<Dynamic<?>, ? extends BaseTreeFeatureConfig> configFactoryIn) {
        this(configFactoryIn, 4, DEFAULT_TRUNK, DEFAULT_LEAF, false);
    }

    public KiwiFruitTreeFeature(Function<Dynamic<?>, ? extends BaseTreeFeatureConfig> configFactoryIn, int minTreeHeightIn, BlockState trunkState, BlockState leafState, boolean vinesGrowIn) {
        ...
        this.setRegistryName(Util.MOD_ID, "kiwi_fruit_tree");
