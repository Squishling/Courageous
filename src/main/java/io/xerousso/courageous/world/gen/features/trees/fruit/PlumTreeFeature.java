...
public class PlumTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
    private static final BlockState DEFAULT_TRUNK = Blocks.OAK_LOG.getDefaultState();
    private static final BlockState DEFAULT_LEAF = ModBlocks.PLUM_LEAVES.getDefaultState().with(HarvestableLeaves.GROWN, false).with(LeavesBlock.PERSISTENT, false).with(LeavesBlock.DISTANCE, 1);
    protected final int minTreeHeight;
    private final boolean vinesGrow;
    private final BlockState trunk;
    private final BlockState leaf;

    public PlumTreeFeature(Function<Dynamic<?>, ? extends BaseTreeFeatureConfig> configFactoryIn) {
        this(configFactoryIn, 4, DEFAULT_TRUNK, DEFAULT_LEAF, false);
    }

    public PlumTreeFeature(Function<Dynamic<?>, ? extends BaseTreeFeatureConfig> configFactoryIn, int minTreeHeightIn, BlockState trunkState, BlockState leafState, boolean vinesGrowIn) {
        ...
        this.setRegistryName(Util.MOD_ID, "plum_tree");
    ...
    protected boolean place(IWorldGenerationReader worldIn, Random rand, BlockPos position, Set<BlockPos> set, Set<BlockPos> set1, MutableBoundingBox mutableBoundingBox, BaseTreeFeatureConfig treeFeatureConfig) {
        ...
        if (position.getY() >= 1 && position.getY() + i + 1 <= worldIn.getMaxHeight()) {
            ...
            if (!flag) {
                return false;
            } else if (isSoil(worldIn, position.down(), (IPlantable) ModBlocks.PLUM_SAPLING) && position.getY() < worldIn.getMaxHeight() - i - 1) {
